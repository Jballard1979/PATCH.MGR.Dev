...
#-- Author:   JBallard (JEB)                                                                                    :
#-- Date:     2022.5.10                                                                                         :
#-- Script:   SCADA-KB.INSTALL.v4.ps1                                                                           :
...
#-- ************************************************************************************************************:
#--
#--&C:\PSTOOLS\PSEXEC64.exe -accepteula -s \\10.165.4.71 POWERSHELL -COMMAND START-PROCESS -FILEPATH C:\TEMP\PATCHES\KB5002537.x64.exe -ARGUMENTLIST /QUIET -WAIT
#--&C:\PSTOOLS\PSEXEC64.exe \\$computer -s -u Adminuser -p AdminPassword msiexec /i C:\TEMP\PATCHES\EDGE.v122.0.2365.80.msi /qb ADVANCED_OPTIONS=1 CHANNEL=100
#--&C:\PSTOOLS\PSEXEC64.exe \\$computer -s -u Adminuser -p AdminPassword msiexec /i C:\TEMP\PATCHES\KB5002537.x64.exe /qb ADVANCED_OPTIONS=1 CHANNEL=100
#--
#--//INVStr = $@"START-PROCESS -WAIT -FILEPATH \\{REMSYSName}\{REMFILEPath} -ARGUMENTLIST “/sAll /rs /msi EULA_ACCEPT=YES AgreeToLicense=Yes RebootYesNo=No” -passthru";
#--//INVStr = $@"START-PROCESS -WAIT -FILEPATH \\{REMSYSName}\{REMFILEPath} -ARGUMENTLIST “/s /v /qn /msi EULA_ACCEPT=YES AgreeToLicense=Yes RebootYesNo=No” -passthru";
#--//PSCmd = $@"&{PSEXECPath} -accepteula -s \\{REMSYSName} POWERSHELL -COMMAND {INVStr}";

#--// PSCmd = $@"&{PSEXECPath} -accepteula -s MSIEXEC /p {REMFILEPath} “/s /v /qn” -passthru";
#--// INVStr = $@"START-PROCESS -WAIT -FILEPATH {REMFILEPath} -ARGUMENTLIST “/q”";
#--// PSCmd = $@"&{PSEXECPath} -accepteula -s \\{REMSYSName} POWERSHELL -COMMAND {INVStr}";
#--
CD\
CLS
